...
use rplcs_events::tournament_1::{
    ChoiceResponse, FightChoices, FightInfo, GambleChoices, MapNodeType, MoveChoices, PlayerState,
};
use std::path::Path;
...
#[derive(Debug, Clone)]
enum PlayerActionRequest {
    Choices(MoveChoices),
    Fight(FightInfo),
    Gamble,
}
#[derive(Debug, Clone)]
enum PlayerActionResponse {
    Choices(ChoiceResponse),
    Fight(FightChoices),
    Gamble(GambleChoices),
}
#[derive(Debug, Clone)]
struct PlayerAction {
    player: usize,
...
impl GameState {
    ...
    pub fn check_game_over(&self) -> Option<GameResult> {
        if self.players.iter().any(|p| p.health == 0) {
            if self.players[0].health == 0 {
                Some(GameResult::Player2Win)
            } else if self.players[1].health == 0 {
